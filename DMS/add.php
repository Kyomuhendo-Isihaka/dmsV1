...
if (!$connection) {
    ...
    if ('addManager' == $action) {
        ...
        if ($fname && $lname && $lname && $phone && $password) {
            ...
            mysqli_query($connection, $query);
            header("location:home.php?id=allPharmacist");
        }
        ...
        if ($b_num && $d_name && $d_description && $mnf_date && $exp_date) {
            $query = "INSERT INTO drugs(batch_num,drug_name,drug_description,manufacturing_date,expiry_date,status) VALUES('$b_num','$d_name','$d_description','$mnf_date','$exp_date','1')";
            mysqli_query($connection, $query);
        ...
        if ($d_name && $d_description && $mnf_date && $exp_date) {
            $query = "UPDATE drugs SET drug_name='{$d_name}', drug_description='{$d_description}', manufacturing_date='{$mnf_date}', expiry_date='{$exp_date}', status='1' WHERE id='{$id}'";
            mysqli_query($connection, $query);
            header("location:home.php?id=allDrug");
